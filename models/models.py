...
class JDELayer(nn.Module):
    ...
    def forward(self, p, out):
        ...
        if self.training:
            ...
            # io[..., :4] *= self.stride
            io = p.clone()  # inference output
            io[..., :2] = torch.sigmoid(io[..., :2]) * 2.0 - 0.5 + self.grid  # xy
            io[..., 2:4] = (
                torch.sigmoid(io[..., 2:4]) * 2
            ) ** 2 * self.anchor_wh  # wh yolo method
            io[..., :4] *= self.stride
            io[..., 4:] = F.softmax(io[..., 4:])
            return (
                io.view(bs, -1, self.no),
...
def attempt_download(weights):
    ...
    msg = (
        weights
        + " missing, try downloading from https://drive.google.com/open?id=1LezFG5g3BCW6iYaV89B2i64cqEUZD7e0"
    )
    ...
    if len(weights) > 0 and not os.path.isfile(weights):
        ...
        # Error check
        if not (
            r == 0 and os.path.exists(weights) and os.path.getsize(weights) > 1e6
