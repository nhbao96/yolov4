...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(
        ...
        if self.rect:
            ...
            self.batch_shapes = (
                np.ceil(np.array(shapes) * img_size / stride + pad).astype(np.int64)
                * stride
            )
        ...
        if cache_images:
            ...
            for i, x in pbar:
                self.imgs[i], self.img_hw0[i], self.img_hw[i] = (
...
def load_mosaic9(self, index):
    ...
    s = self.img_size
    indices = [index] + [
        random.randint(0, len(self.labels) - 1) for _ in range(8)
