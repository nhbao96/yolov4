...
def labels_to_image_weights(labels, nc=80, class_weights=np.ones(80)):
    ...
    class_counts = np.array(
        [np.bincount(labels[i][:, 0].astype(np.int64), minlength=nc) for i in range(n)]
    )
...
def build_targets(p, targets, model):
    ...
    for i, j in enumerate(model.yolo_layers):
        ...
        anchors = (
            model.module.module_list[j].anchor_vec
            if multi_gpu
            else model.module_list[j].anchor_vec
        )
...
def select_best_evolve(
    path="evolve*.txt",
):  # from utils.utils import *; select_best_evolve()
...
def kmean_anchors(
    ...
    wh *= np.random.uniform(
        img_size[0], img_size[1], size=(wh.shape[0], 1)
...
def plot_results(
    ...
    if bucket:
        ...
        files = [
            "https://storage.googleapis.com/%s/results%g.txt" % (bucket, x) for x in id
        ]
    else:
        ...
        try:
            ...
            for i in range(10):
                ...
                if i in [0, 1, 2, 5, 6, 7]:
                    ...
                    # y /= y[0]  # normalize
                ax[i].plot(
                    x, y, marker=".", label=Path(f).stem, linewidth=2, markersize=8
                )
                ax[i].set_title(s[i])
